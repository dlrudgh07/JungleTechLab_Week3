...
{
	FTransform(){ }
	// 오일러로 들어오면 쿼터니언으로 변환해 보관. 기존 호출부 호환용
	FTransform(FVector _Location, FRotator _Rotation, FVector _Scale)
		: Location(_Location), Rotation(FQuaternion::FromEuler(_Rotation)), Scale(_Scale)
	{
	}
	FTransform(FVector _Location, FQuaternion _Rotation, FVector _Scale)
